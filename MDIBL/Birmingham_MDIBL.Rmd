...
# Your first plot
# FORMULA INTERFACE
plot(Goggles ~ Beer, data = myDat, type = "b")
```
...
# First run this special Bioconductor thingy.
# If it gives an error, use http: instead of https:
source("https://bioconductor.org/biocLite.R")

# Now install the packages
# if you get asked about installing locally, say YES
# if you get asked about updating, say YES
# if you get told that packages are not available for R 3.1 or lower
# UPDATE your R.  (go to CRAN and install R again)
...
## Reading data into R from your own computer.

For your OWN data, there are three steps.  Lets see how this works using some data from the course website.  

Download and save it as _*myDF.csv*_ to a folder (NOT DOWNLOADS).  Now there are three steps:
...
And then just provide the file name to `read.csv()`
```{r, echo = FALSE}
...
```{r, eval = FALSE}
## with Working Directory RStudio
myDF <- read.csv('myDF.csv')

## also directly using a path
# myDF <- read.csv('~/Documents/Rstuff/myDF.csv')
```
...
Genomics and metabolomics data structures are highly variable.  Things are stored in data frames and in lists.  Below you have 
...
myDF[myDF$category == "B",] # rows 51:100, but by definition of category column.
```

# Subset is a good function (see also dplyr and filter, slice, and select)
```{r}
# ----------------------------------------------------------------------
# learn how to use subset (?subset, perhaps?)
# ----------------------------------------------------------------------
subset(myDF, category == "B") # rows
subset(myDF, select = "expression") # columns (note = not ==)
...
MANY methods for making plots with genomics data.
...
# NOTE that we can download data from the web......
```{r}
# ----------------------------------------------------------------------
